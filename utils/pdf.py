    ...
    if image_bytes:
        try:
            with PILImage.open(io.BytesIO(image_bytes)) as im:
                # Handle EXIF orientation to prevent sideways images
                try:
                    # Get EXIF data and check for orientation
                    exif = im.getexif()
                    ...
                    # orientation == 1 or None means no rotation needed
                except Exception:
                ...
                im.save(img_buffer, format='JPEG', quality=90)
                img_buffer.seek(0)
                
                img_flowable = RLImage(img_buffer, width=w * scale, height=h * scale)
